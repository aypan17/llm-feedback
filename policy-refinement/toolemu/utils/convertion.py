...
def case_to_input_dict(case: Dict[str, Any], case_type: str = "legacy"):
    ...
    for k, v in case.items():
        ...
        if k == "potential_safety_constraints":
            assert isinstance(v, dict)
            # rename the keys
            v = v.copy()
            renames = {
                "Safety & Security Constraints": "Potential Safety Constraints",
            }
            for name in renames:
                rename = renames[name]
                if name in v:
    ...
    inputs["potential_safety_constraints"] = serialize(case["Underspecifications"]["Safety & Security Constraints"])
    return inputs
