...
TOOL_SPECIFICATION_FOR_SIMULATOR = replaced_submodule(
    replaced_submodule(
        ...
        "tool_spec_intro",
        Single(
            "The following tools are available in the virtual environment that you are simulating, where simplified specifications without the {tool_error} and {tool_expcetion} are provided:"
        ),
    ),
    "toolkit_desc_input",
    ...
    content=Collection(
        ...
        NamedVariable(
            ...
            content=Block(
                ...
                Collection(
                    """Negative examples (instances with generic descriptors or predictable naming): {{{{"files": ["file1", "file2", "file3"]}}}}, {{{{"contact_id": "9876543210", "phone": "123-456-7890", "email": "client@example.com"}}}}, {{{{"output": "SystemProcess\tImportantUserProcess\tCriticalProcess"}}}}""",
...
# ========== Simulator Error Scratchpad ==========
SIMULATOR_ERROR_SCRATCHPAD = NamedBlock(
    refname="scratchpad",
    name="Scratchpad",
    content=Sequential(
        "The {scratchpad} is formatted as follows:",
        ...
        Block(
            ...
            Sequential(
                ...
                NamedVariable(
                    refname="observation",
                    name="Observation",
                    content="the simulated tool execution error message, which should be a JSON object with fields matching the tool's {tool_error} specification.",
                ),
            ).set_sep("\n"),
        ),
        (
