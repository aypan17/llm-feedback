...
class FunctionTool(BaseTool):
    ...
    def create_description(self, detail_level="high") -> str:
        ...
        if detail_level not in ["none", "low", "medium", "high", "error"]:
            raise ValueError(
                f"detail_level must be one of 'none', 'low', 'medium', 'high', 'error'"
            )
        include_desc = detail_level in ["medium", "high", "error"]
        include_args = detail_level in ["low", "medium", "high", "error"]
        include_returns = detail_level in ["low", "medium", "high"]
        include_exceptions = detail_level in ["high"]
        include_errors = detail_level in ["error"]
        ...
        if include_errors:
            desc += get_combined_desc("Errors", self.errors)
